...
export default tool({
  ...
  args: {
    ...
    kind: tool.schema.string().optional().describe(
      "AST node kind to match (e.g. 'function_declaration', 'arrow_function'). Alternative to --pattern.",
    ),
    paths: tool.schema.array(tool.schema.string()).optional().describe(
      "File paths or directories to search. Defaults to '.'",
    ),
    lang: tool.schema.string().optional().describe(
      "Language (e.g. 'ts', 'js')",
    ),
    rewrite: tool.schema.string().optional().describe(
      "Rewrite pattern for replacement",
    ),
    filesWithMatches: tool.schema.boolean().optional().describe(
      "Print only file paths with matches",
    ),
    json: tool.schema.boolean().optional().describe(
      "Output matches as JSON",
    ),
    context: tool.schema.number().optional().describe(
