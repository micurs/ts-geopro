...
function usage(exitCode = 1): never {
  ...
  console.error(
    "  create <title> <body> <head_branch> [base_branch]   Create pull request",
  );
  console.error(
...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new GiteaApiError(
      response.status,
      `Gitea API error (${response.status}): ${message}`,
    );
  }
...
async function createPullRequest(args: string[]): Promise<void> {
  ...
  if (!title || !body || !head) {
    console.error(
      "Usage: gitea-pr.sh create <title> <body> <head_branch> [base_branch]",
    );
    console.error(
      'Example: gitea-pr.sh create "My PR" "Description" "micurs/my-branch" "main"',
    );
    Deno.exit(1);
  }
...
async function readStdinBody(): Promise<string> {
  ...
  if (!body) {
    throw new Error(
      "Comment body cannot be empty. Pipe a markdown file into stdin.",
    );
  }
  return body;
}
...
async function findCurrentPullRequest(): Promise<GiteaPull> {
  ...
  const pull = pulls.find((candidate) =>
    ...
    candidate.head?.label?.endsWith(`:${branch}`)
  );
...
async function listReviewComments(
  ...
  for (const review of reviews) {
    ...
    comments.push(
      ...await request<GiteaReviewComment[]>(
        `pulls/${prNumber}/reviews/${review.id}/comments`,
      ),
    );
  }
  return comments;
...
async function addReviewComment(args: string[]): Promise<void> {
  ...
  if (!sourceFile || !Number.isInteger(lineNumber) || lineNumber < 1) {
    console.error(
      "Usage: gitea-pr.sh comment <source_file> <line_number> < comment.md",
    );
    Deno.exit(1);
  }
...
async function findReviewCommentById(
  ...
  const comment = comments.find((candidate) =>
    String(candidate.id) === commentId
  );

  if (!comment) {
    throw new Error(
      `Review comment #${commentId} was not found on PR #${prNumber}.`,
    );
  }
  if (!comment.path || !comment.position || !comment.pull_request_review_id) {
    throw new Error(
      `Review comment #${commentId} is missing path, line, or review metadata.`,
    );
  }
...
async function replyToCommentViaWebForm(
  ...
  const response = await fetch(
    `${giteaUrl}/${repo}/pulls/${prNumber}/files/reviews/comments`,
    {
      method: "POST",
      redirect: "manual",
      headers: {
  ...
  if (response.status !== 303 && response.status !== 302) {
    ...
    throw new Error(
      `Gitea web form error (${response.status}): ${text.slice(0, 200)}`,
    );
  }
  ...
  console.log(
    `✓ Reply added to PR #${prNumber} for comment #${commentId} using web form endpoint`,
  );
  console.log(
    `  URL: ${giteaUrl}/${repo}/pulls/${prNumber}#issuecomment-${commentId}`,
  );
}
...
async function replyToComment(args: string[]): Promise<void> {
  ...
  try {
    ...
    console.log(
      `✓ Reply added to PR #${pull.number} for comment #${commentId}`,
    );
    if (reply.html_url) {
